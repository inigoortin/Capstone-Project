...
def get_pie_chart(entered_site, payload_range):
    # Filtrar primero por el rango de carga útil
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(payload_range[0], payload_range[1])]
    
    if entered_site == 'ALL':
        # Si 'ALL' es seleccionado, agrupar por sitio de lanzamiento
        ...
        # Calcular el número de éxitos y fracasos
        pie_data = filtered_df.groupby('class').size().reset_index(name='count')
        # Crear gráfico circular para el sitio seleccionado
        fig = px.pie(pie_data, 
                     values='count', 
                     names='class',  
...
def get_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        # Si se seleccionan todos los sitios, mostrar todos los datos
        fig = px.scatter(spacex_df, x='Payload Mass (kg)', y='class', 
                         color='Booster Version Category', 
                         title="Payload Mass vs. Launch Success for All Sites")
    else:
        # Si se selecciona un sitio específico, filtrar los datos
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        # Mostrar los datos solo para el sitio seleccionado
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', 
                         color='Booster Version Category', 
                         title=f"Payload Mass vs. Launch Success for site {entered_site}")
    return fig
